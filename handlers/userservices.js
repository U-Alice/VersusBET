...
const { password, password } = require("../database/passwordreset");
...
module.exports.recover = (db)=>{async (req, res) =>{
   const user = User.findOne({email: req.body.email, password: req.body.password})
    if(!user){
        res.status(400).json({message:'user not found'})
    }
//    await authenticateToken().catch(console.error()) 
     const password = new password({
      email: User.email,
      _id: User._id
  })
  password.resetPasswordToken()
  password.save().then(user => {
